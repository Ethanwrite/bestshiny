...
| 2.10 | **Fixed 2026-08-29 — the full check moved to an async verifier; the upload path stayed thin.** Direct uploads register `PENDING_VERIFICATION` (migration `0058`); `verify_pending_assets` claims each to VERIFYING under a lease (a crashed worker's claim lapses and the row re-verifies), recomputes the stored object's SHA-256, fully decodes images (Pillow `load`) and videos (ffprobe, then `ffmpeg -f null` over the whole stream), and writes READY, INVALID (does not decode) or QUARANTINED (bytes contradict the declaration — forged MIME, SHA mismatch; one is a broken export, the other is tampering, and operators need to know which). Rejected evidence objects remain retained and charged to workspace storage until an explicit deletion lifecycle removes them; this prevents repeated invalid uploads from bypassing quota. `reference_url` refuses anything not READY before a provider is billed. Truncated JPEG/PNG, corrupt MP4, forged MIME, SHA mismatch and crash recovery are tested. | `services/media-service/media_service/verification.py` |
