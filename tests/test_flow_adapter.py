...
from production_domain.models import ProviderAccount
...
async def test_flow_adapter_maps_video_without_leaking_upstream_domain(tmp_path):
    runtime = FakeRuntime()
    provider = GoogleFlowProvider(runtime, Settings(_env_file=None, flow_project_id="flow-project"))
...
async def test_flow_uses_account_project_id_for_submit_upload_and_poll(container, tmp_path):
    ...
    provider = GoogleFlowProvider(runtime, container.settings, container.database)
...
async def test_flow_explicit_bad_request_is_safe_to_retry():
    ...
    provider = GoogleFlowProvider(
        BadRequestRuntime(), Settings(_env_file=None, flow_project_id="flow-project")
    )
