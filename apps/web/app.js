...
async function renderShotStage() {
  ...
  if (!committed) {
    ...
    if (generating) {
      // A live frame: sheen across the stage, a pulsing frame and a sliding bar while the provider renders.
      const active = state.candidates.filter((candidate) => ["QUEUED", "RUNNING", "GENERATING", "VALIDATING"].includes(candidate.status)).length;
      stage.innerHTML = `
        <div class="generating-stage" role="status" aria-live="polite">
          <div class="frame-pulse" aria-hidden="true"></div>
          <strong>Generating this shot</strong>
          <p>${active} variant${active === 1 ? "" : "s"} rendering. They appear below as the provider returns them.</p>
          <div class="job-progress indeterminate"><i></i></div>
        </div>`;
      return;
    }
    stage.innerHTML = `
      <div class="empty-block">
        <span class="empty-icon" aria-hidden="true">◻</span>
        <strong>Nothing generated for this shot</strong>
        <p>Generate the shot to see the take here, then approve one variant into the timeline.</p>
      </div>`;
    return;
  }
  ...
  return anchors.map((anchor) => {
    ...
    return `
      <figure class="asset-card" data-anchor-asset="${anchor.media_asset_id || ""}">
        <div class="asset-thumb${thumbClass}" data-anchor-thumb="${anchor.id}">${thumbLabel}</div>
        <figcaption>
...
async function creativeApproveBeats() {
  ...
  state.creative.beatEdits = {};
  toast(`Compiled ${result.shot_ids.length} shots from screenplay revision ${result.screenplay_revision}. Continue in Director.`);
  await openCreativeSession(view.session.id);
  await selectProject(state.project.id);
