...
class AssetRegistry:
    ...
    def promote(
        ...
        *,
        ...
        with self.database.session() as session:
            ...
            self._assert_canonical_provenance(session, version)
    ...
    @staticmethod
    def _assert_canonical_provenance(session, version: AssetVersion) -> None:  # type: ignore[no-untyped-def]
        """Prevent low-trust generated bytes from being laundered through a new version.

        Generation routing is only the first trust boundary. A user may later wrap
        the same ``MediaAsset`` in a USER_UPLOAD version, so canonical promotion
        must inspect the immutable media provenance as well as server-authored
        generation metadata.
        """

        declared_trust = version.metadata_json.get("provider_trust_level")
        if declared_trust is not None:
            try:
                declared_allowed = provider_can_handle(
                    ProviderTrustLevel(str(declared_trust)),
                    AssetCriticality.CANONICAL,
                )
            except ValueError as exc:
                raise AssetVersionNotPromotable(
                    "generated asset version has invalid provider trust provenance"
                ) from exc
            if not declared_allowed:
                raise AssetVersionNotPromotable("low-trust generated assets cannot become canonical")
        ...
        media_ids = {
            ...
            for media_id in [
                ...
                *session.scalars(
                    select(AssetVersionMedia.media_asset_id).where(
                        AssetVersionMedia.asset_version_id == version.id
                    )
                ),
            ]
            if media_id
        }
        media_assets = [session.get(MediaAsset, media_id) for media_id in media_ids]
        try:
            assert_canonical_media_provenance(
                session,
                (media for media in media_assets if media is not None),
            )
        except CanonicalMediaProvenanceViolation as exc:
            raise AssetVersionNotPromotable(str(exc)) from exc

        if version.source == "PASSENGER_GENERATION" and not media_ids:
            raise AssetVersionNotPromotable("generated asset version has no media provenance")
