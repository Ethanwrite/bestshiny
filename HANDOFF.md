...
> residuals; F13 is a product decision, §1.19 — decided the same day: the reset flow is closed and
> its entry hidden in production until a delivery channel exists. No migration; the schema head stays `0081`. The
...
> schema head stays `0081`. The host nginx in front of the web container gained the same `Upgrade`/`Connection`
> headers (both public blocks, plus an http-level map in `conf.d`) at ≈11:51Z on the operator's request, so
> the worker WebSocket now upgrades end to end through `/api/`. Committed as `341faf1`, opened as
...
## 1g. 2026-08-27 — video references: validated, adapted, or refused by name
