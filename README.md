...
历史升级前审计保留在 [CURRENT_ARCHITECTURE.md](CURRENT_ARCHITECTURE.md)，当前 Visual Runtime 的逐阶段实施记录见 [docs/VISUAL_RUNTIME_IMPLEMENTATION.md](docs/VISUAL_RUNTIME_IMPLEMENTATION.md)，源码与 Skill 审计见 [docs/source-audit.md](docs/source-audit.md) 和 [docs/skill-research.md](docs/skill-research.md)。
...
- Model Registry/Router：JSON 配置的模型版本、能力、成本、延迟与失败先验；输出可解释候选、扣分和降级原因。
...
- Credits estimate：显示供应商预估成本、分辨率/参考图/服务倍率和积分估算版本；尚不是用户钱包或真实扣费账本。
...
当前迁移链已追加到 `0020_provider_media_upload_claim`；历史数据会原地升级，不会替换已有项目、镜头、媒体、身份、候选、QA 或成本表。旧版 `local@ai-director.invalid` 工作空间升级后保持隔离，普通注册不会自动认领；如需转移，必须调用下文说明的受保护内部接口。迁移链已在 PostgreSQL 17 + pgvector 0.8.6 上验证 fresh install、从 `0012` 升级、JSON embedding 转 `vector(16)` 与 `alembic check`。
...
- Credits 目前是透明估算算法和生成成本记录，不包含用户余额、充值、扣费、退款、并发预占或账务对账。
