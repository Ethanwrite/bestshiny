...
### 1.19 Production password reset sends nothing (audit F13)

`POST /api/auth/password-reset/request` answers "如果该邮箱存在，重置说明已发送" in every environment,
but only `development`/`test` return the token in the body; production creates and hashes the
token and discards it. There is no mail transport anywhere in the repository (no SMTP, no
provider SDK, no outbox), so a production user who clicks *Forgot password* cannot obtain the
token the dialog asks for. The mechanism behind it - hashed, one-use, expiring tokens; sessions
revoked on reset - is implemented and tested; only delivery is missing. Left unchanged on
2026-09-06 because it is a product decision, not a fix: either choose a mail provider (credentials
are yours to supply; the send belongs behind a durable, retried outbox like the memory index's) or
...
### 1.7 Product decision: deterministic fallback — decided 2026-09-08

Decided by the 2026-09-08 Skill-runtime instruction: **the deterministic fallback is retained, and a
fallen-back outcome is never marked Skill-driven.** When the prompt-compiler model returns invalid JSON,
fails re-verification or is unavailable, the deterministic package stands in and the compilation record
carries `execution_mode: DETERMINISTIC`, `skill_driven: false` and the `fallback_reason`
(`prompt_compilations.diff_json.skill_invocation`). A fresh Skill verdict of `NOT_COMPILABLE` is not
overridden: generation refuses with the Skill's reason until the shot changes or the stage reruns.
Record: §2.52 and `docs/SKILL_RUNTIME.md` §6.
...
| 2.27 | **Fixed 2026-09-01 — request-dependent duration ceilings are routing exclusions.** No migration was needed: the router (v3) reads the ceilings from the `provider_metadata.modes` JSON the registry already records. `_mode_failures` resolves the derived task type to its mode declaration (V2V falls back to `r2v`, which is where providers that consume a reference video declare it), applies `max_duration` / `min_duration`, and substitutes `max_duration_with_reference_video` when the shot carries one — so a 12-second reference-video shot is now `DURATION_UNSUPPORTED` in `RouterDecision.rejected`, before a Job exists, instead of an adapter refusal after routing. **Residual:** only Wan 2.7 declares `modes` today; every other profile keeps the single profile-level bound, and the adapter enforcement stays as the last line. Original text: the router's duration gate could not express a request-dependent ceiling, and the Wan 2.7 R2V 10s cap failed at the adapter. | `core/model-registry/model_registry_core/router.py` |
| 2.28 | **Fixed 2026-09-01 — per-mode roles and material combinations are routing exclusions.** The router (v3) reads `provider_metadata.modes.<mode>.accepts` and `.material_combinations` for the mode a request resolves to: a required media role the mode does not accept is `MODE_ROLE_UNSUPPORTED`, and a role set outside the mode's closed combination list is `MODE_COMBINATION_UNSUPPORTED`, both in `RouterDecision.rejected` before a Job exists. This mattered the moment the scene-champion table existed: reference images plus an end frame is a routine production shot, it reads as the `first_last_frame` scene, and Wan 2.7 — that scene's primary — resolves it to an R2V mode that accepts no last frame, so the table would have pinned a pick the adapter refused on every attempt (open scoring never chose Wan, its priors being all 0.5). The adapter's `_I2V_COMBINATIONS` stays as the last line and still pins the registry declaration. **Residual:** only Wan 2.7 declares `modes`; a profile-wide flag such as `supports_end_frame` is still the only routing fact for every other model, and the role vocabulary the router derives covers first/last frame, reference image and reference video — not `first_clip`, `driving_audio` or `reference_voice`, which have no `requires_*` flag. Original text: I2V's material-combination table was held in the adapter, and the router's per-axis booleans could not represent "these axes only in these combinations". | `core/model-registry/model_registry_core/router.py`, `providers/wan/wan_provider/adapter.py` |
...
| 2.38 | **Fixed 2026-09-02 — the model path is exercised by tests that simulate a successful DIRECTOR reply, under the Director Skill.** `tests/test_creative_director.py` drives `ScriptedDirector` through the new `DirectorTurnResult` and `Screenplay` contracts (corrections, character edits, the inference guard, assumptions, an original treatment/screenplay, rewrites, malformed output). What remains unexercised is a *live* opus-5 reply against the real protocol: run one paid turn and one paid screenplay draft on dev before presenting the screenplay stage to paying users (see §2.46). | `core/creative-director/creative_director_core/service.py` |
...
| 2.45 | **Fixed 2026-09-02 — the director now answers in its own words, images no longer carry video parameters, and conversations and never-generated shots can be deleted.** The `DIRECTOR` model was called and paid every turn but only for a JSON field patch; the user saw one of two fixed English sentences (production: six paid opus-5 turns, one sentence). `CreativeDirectorService._model_patch` asks for `{"reply","fields"}` with the last twelve turns as conversation and uses the reply as the turn's content (fixed sentence only as the fallback); an unmistakable typed approval on a proposed brief approves it. The create form sent the video resolution for images, which OpenRouter refused (`invalid_value` on `resolution`); the field is video-only now, ratios are per medium, and `GenerationGateway._submit` states the priced resolution for video jobs only. `DELETE /v1/creative/sessions/{id}` (→ ABANDONED) and `DELETE /v1/shots/{id}` (never-generated shots only, chain re-joined). **Residuals:** the rules engine's clarifying questions are still English; image quality is the server-owned tier, not a user-selectable level; a shot with any paid history cannot be deleted, only left in place. | `core/creative-director/creative_director_core/service.py`, `apps/api/video_platform_api/creative_routes.py`, `apps/api/video_platform_api/main.py`, `services/generation-gateway/generation_gateway/gateway.py`, `apps/web/app.js`, `apps/web/index.html` |
| 2.46 | **Done 2026-09-02 — the creative director now writes; residual risks recorded here.** The DIRECTOR model runs under `skills/director/SKILL.md` (content hash on every turn and screenplay row), sees the whole conversation with per-field provenance and question states, applies explicit SET/REPLACE/UPSERT/REMOVE/KEEP operations (an inference cannot overwrite a user fact), and authors versioned screenplay revisions after a server-enforced brief approval (CLARIFYING refused, CRITICAL gaps must be answered or their assumption accepted, assumed values confirmed). Key visuals derive from brief + screenplay as content-versioned anchors; the bible lock creates `CharacterIdentityVersion` rows and a `ProjectStyleLock` through the existing services and blocks compilation on failure; compiled shots carry `creative_shot_lineage`. One round is one transaction; `client_turn_id` replays. Migration `0070`. **Residual risks (not verified here):** (1) no live DIRECTOR call has run against the new JSON contracts — opus-5 may return prose around the JSON (the parser strips code fences only) or a screenplay that fails validation, which degrades to the labelled scaffold on every attempt; watch `reason_codes` on production turns and screenplay rows after deploy. (2) The compiler's actor regex reads one leading Latin token: multi-word Latin names are rendered as one hyphenated token (`Lin-Jin`) and the Character row is created under that script name; CJK names longer than four characters will not parse as actors. (3) A project keeps one style lock: a second session inherits it on record instead of locking its own style plate. (4) The frontend was built (`vite build`) and its state machine follows the API, but no browser session exercised the new page end to end. (5) The Director Skill says framing is not decided at this stage while the screenplay contract asks for a suggested `shot_type`; the protocol marks it as a suggestion the shot planner may refine. (6) Beat-time edits reach the screenplay through `apply_beat_edits` (dialogue text, staging note, shot type, duration, states) — a structural edit (new shot, new beat) must go through `POST …/screenplay/edit` before approval. (7) The "thinking… then words arrive" behaviour is a client-side progressive reveal of a completed reply; `ModelRoleRuntime.execute_chat` returns one settled JSON object, so true token streaming would need a streaming runtime path with its own spend settlement. (8) Before the lock a user can regenerate a key visual with direction or bind an uploaded image (`…/anchors/{id}/regenerate`, `…/replace`); both create a new anchor version and invalidate a DRAFT bible. | `core/creative-director/`, `apps/api/video_platform_api/creative_routes.py`, `apps/web/app.js`, `migrations/versions/0070_creative_director_screenplay.py` |
| 2.47 | **Fixed 2026-09-04 — the creative-director audit's P1/P2 findings.** (1) The approved brief's `aspect_ratio` rides every shot intent; the prompt compiler and the generation request read it before `project.default_aspect_ratio` (`platform_contracts.approved_aspect_ratio`). (2) `_materialize_beats` merges the screenplay's own `shot.anchors` ahead of the derived keys instead of overwriting them. (3) `required_copy` placed in a beat/shot that does not exist fails screenplay validation, like any other dangling reference. (4) A USER_STATED operation must quote the user *and* the quote must say the value: fact paths literally, enum-like paths through the extractor's cues, numbers in digits or Chinese numerals; otherwise `VALUE_NOT_IN_EVIDENCE`, demoted to MODEL_INFERRED. Prose paths (logline, hook, CTA) keep the quote check only. (5) A bible whose brief asks for a look other than the project's locked one is refused with `STYLE_LOCK_CONFLICT` until `accept_inherited_style` is sent; the draft bible carries `style_inheritance`, the skipped plate reads `PROJECT_STYLE_LOCK_DIFFERS`, the lineage records the acceptance. A lock made outside the director (no recorded look) always needs the decision. (6) Prohibitions are enforced on every screenplay surface (actions, staging, summaries, scenes, treatment), by the terms the sentence forbids (`prohibited_terms`), skipping mentions that are themselves negated; only instruction-shaped sentences count (`enforceable_prohibition` - 别墅 is a villa). They also ride every shot intent into the compiler as constraints, negative-prompt terms and a `prohibition=` QC line. Synonyms are still not understood deterministically; the prompt/QC path is the semantic backstop. (7) A call to action counts only in what compiles: placed copy, a spoken line or an action's staging - never the treatment's ending alone. (8) A user-stated selling point missing from a commerce piece is BLOCKING (severity by provenance, like the product); selling points are preserved as product claims on the user's authority (`preserved_product_claims`) whatever `must_preserve` the director wrote, on the product shots and on the ledger. (9) `creative_turn_claims` (migration `0080`, `REQUIRED_SCHEMA_REVISION` moves with it): a `client_turn_id` is claimed before the director call, so a duplicate in flight is `TURN_IN_PROGRESS` (409, retryable) and pays nothing; the claim is deleted with the turn, freed on failure, taken over after a five-minute lease. Covers the session-create window too. (10) Two cast names that collapse to one script token (`Mary Jane` / `Mary-Jane`, case included) fail validation instead of sharing a Character row. (11) An invariant scoped to an unknown character or scene fails validation instead of applying nowhere. (12) `_bind_scene_locations` matches the anchor's raw location against the compiler's cleaned heading spelling (`compiler_location`), so a punctuated location still binds its plate. (13) The SHOT_RESULT memory names its entities (bound character ids, their canonical CHARACTER assets, every canonical asset whose plate the shot was bound to), which is what the production runtime filters retrieval on. (14) `MultimodalMemoryEngine.reindex` re-embeds a degraded memory in place; the outbox retry uses it instead of appending a second row. (15) A callback that reports only some of a job's characters leaves the submission ACCEPTED with `missing_character_ids`; REPORTED needs every requested character, and the accepted deadline settles the rest (`NO_REPORT_BEFORE_DEADLINE`, reconciliation). The webhook answers `complete` and `missing_characters`. Tests: `tests/test_audit_p1_p2_fixes.py`. | `core/creative-director/`, `core/video-prompt/video_prompt_core/compiler.py`, `core/production/director_production/pipeline.py`, `core/memory/memory_core/`, `services/character-evidence/character_evidence/tracking.py`, `apps/api/video_platform_api/main.py`, `migrations/versions/0080_creative_turn_claims.py` |
| 2.48 | **Fixed 2026-09-06 — reference images reach Seedream; the shot contract is one dominant action plus a line; execution duration is the router's; identity references narrow to the faces that matter; both multimodal embeddings are on, as advice.** (1) `ArkProvider.generate_image` filtered its payload through an allowlist that never mapped the Gateway-resolved `start_frame_url` / `end_frame_url` / `reference_urls` onto Seedream's `image` parameter, so every reference-bearing Seedream request was billed and executed as plain text-to-image - invisible to a suite whose URL-mode contract test recorded the Gateway through a fake provider. The adapter now builds `image` (one URL as a string, several as a list), refuses a local asset id and more than the registry's declared `max_reference_images` (6, pinned by a drift test), and an end-to-end test uploads a PNG through `POST /v1/assets`, generates with `reference_asset_ids`, and reads the signed URL back off the Ark request. The web app shows the picked reference on the canvas at once (an object URL, before any upload), gates on the server's own PNG/JPEG/WebP extension+MIME table (`REFERENCE_IMAGE_TYPES`, pinned to `media_validation._IMAGE_TYPES` by a test) instead of `image/*`, and an upload refusal names the HTTP status and the server's `detail`. **Residual:** Seedream's own per-request reference ceiling is not recorded in this repository (the registry's 6 is a `MANUAL_PRIOR`); the adapter enforces the declared bound, and the first live reference edit should confirm the vendor accepts a list on `image` (the video path's `role` vocabulary does not apply to the images API). (2) `ScreenplayShot`: one dominant `action` plus 0..1 `dialogue` (a line alone is a speaking shot; neither is refused); `micro_actions` (`blink`, `breathe`, `mouth_movement`, `gaze_shift`, `slight_head_turn`, aliases in Chinese and English) ride along and never count; a staging note that sequences actions (然后/接着/then/after that/...) is refused; the caps are named constants (`MAX_SCENES`/`MAX_BEATS`/`MAX_SHOTS_PER_BEAT` = 12/40/12, `MIN`/`MAX_SHOT_DURATION_SECONDS` = 1/15, `requested_duration` accepted as an alias). A line spoken during an action rides on the intent (`director_intent_json.dialogue`) and the prompt compiler reads it there. The protocol text says all of this; the old "action OR dialogue, never both" and the drifted "2-10 seconds" are gone. (3) `estimated_speech_seconds` (4 CJK characters/s, 2.5 words/s) must fit `usable_dialogue_window` (duration minus 0.5 s at each end) or the shot is refused - at model output, at `POST …/screenplay/edit` and at beat-time edits alike. (4) `present_characters` (everyone in frame, staged by the prompt as an `in frame:` constraint; they count for scoped invariants and key visuals) versus `identity_critical_characters` (≤ `MAX_IDENTITY_CRITICAL_CHARACTERS` = 4, a subset of present; both default to the actor and the speaker). Only identity-critical characters become `character:` anchors, and `platform_contracts.identity_critical_subjects` narrows the frame-anchor planner's subjects to them in both the production runtime's reference merge and the pipeline's binding set - a declaration matching none of the planner's subjects falls back to all of them rather than rendering with no face held. `MAX_CAST` is unchanged. (5) `model_registry_core.duration`: the router plans an execution length per candidate (`EXACT`, `SNAP_UP` to the shortest legal length - a declared step or the minimum - never down; `SPLIT_SHOT` only ever on a rejected model, with the segment plan in the rejection and in the `LookupError`). `ModelCandidate` / `RouterDecision` carry `execution_duration` / `execution_strategy` / `execution_segments`; the planner submits, quotes and records (`routing_context.duration_seconds`, `requested_duration_seconds`, `execution_strategy`) the execution length while the canonical shot keeps the request; a retry onto an alternative uses that alternative's own length; passenger admission does the same for a named or Auto video model and refuses an over-ceiling request with the split plan before any reservation. Veo's `[4, 6, 8]` is declared as `capability_profile.provider_metadata.supported_durations` on the three OpenRouter Veo profiles; **migration `0081_veo_discrete_durations`** writes it onto existing rows (data only; `REQUIRED_SCHEMA_REVISION` moves with it). **Residual:** running one shot as several provider calls is not built - a SPLIT_SHOT is reported, and the shot has to be split at the screenplay (per-shot MP4 delivery has no assembly step); only Veo declares a step set, every other model keeps its range. (6) `MULTIMODAL_EMBEDDING` gains a FALLBACK binding to `gemini-embedding-2-openrouter` (priority 10; the startup sync inserts the missing row, no migration), and `ModelRoleRuntime._select` walks the bindings in order and skips a provider the capability catalogue refuses for want of a credential - before this the PRIMARY was taken and the refusal raised, so a FALLBACK could never serve. The OpenRouter adapter translates Voyage-shaped multimodal pieces (`image_url` string / `image_base64` data URI) into its own `{"url": …}` content shape and drops the Voyage-only `input_type`; a video piece is refused as at Voyage. `FEATURE_VOYAGE_MEMORY` now defaults on; `FEATURE_SEMANTIC_STYLE_ADVISORY` (new, default on) builds the `STYLE_SEMANTIC_EMBEDDING` embedder on a recorded or live transport in **advisory** mode: computed and recorded on the lock (`semantic_layer_mode`, `semantic_layer_absent_reason`) and on every candidate evaluation (`STYLE_SEMANTIC_ADVISORY:<verdict>`), never a refusal and never the verdict; `FEATURE_SEMANTIC_STYLE_LOCK` keeps the enforced two-layer gate and wins when both are set, and a lock made under the enforced gate is judged enforced whatever this process runs. Gemini usage (`prompt_tokens`) settles through the same `TokenCostEngine` path as Voyage's pixels. The retired OpenRouter Voyage row stays disabled and unbound. **Residual:** neither Gemini embedding call nor a Voyage call has run live from this tree; OpenRouter's acceptance of `dimensions` for `google/gemini-embedding-2` is the pre-existing (untested) contract; memory retrieval only matches vectors from the same provider/model/dimension, so a fallback-embedded memory is found only by a fallback-embedded query (§2.23 unchanged). Tests: `tests/test_ark_image_references.py`, `tests/test_reference_upload_contract.py`, `tests/test_shot_contract_v2.py`, `tests/test_execution_duration.py`, `tests/test_multimodal_embedding_enablement.py`. | `providers/seedance/seedance_provider/adapter.py`, `apps/web/app.js`, `core/creative-director/creative_director_core/{schemas,screenplay,beats,director_context}.py`, `core/video-prompt/video_prompt_core/compiler.py`, `packages/contracts/platform_contracts/shot.py`, `core/model-registry/model_registry_core/{duration,router,schemas}.py`, `services/production-engine/production_engine/runtime.py`, `core/entitlements/entitlement_core/{admission,runtime,service}.py`, `core/style/style_core/service.py`, `providers/openrouter/openrouter_provider/adapter.py`, `config/model-registry/defaults.json`, `migrations/versions/0081_veo_discrete_durations.py` |
...
| 2.49 | **Fixed 2026-09-06 — the production-workflow audit's ten confirmed defects.** Verified against `685f891` (the audit's own fault-injection script reproduced all ten; one more, F11, was already closed by `5010ce5`). (F01) With Alibaba OSS (`S3_VERIFY_UPLOAD_SHA256_ON_COMPLETE=true`, the deployed mode) a direct upload's presigned PUT targeted the content-addressed key, so a client that declared another upload's digest overwrote *that* object - in any project - before completion read the hash; the victim's row stayed READY. In that mode the PUT now targets a per-upload slot (`{sha[:2]}/{sha}-{upload_id}{suffix}`), a same-project duplicate is completed from the adopted object with **no** write credential (`url: null`, `existing_asset_id` set), and a replay never re-presigns an adopted key. The store-enforced mode (`x-amz-checksum-sha256` honoured) keeps the content-addressed key unchanged. (F02) The relayer read its nonce from the node's pending count while a persisted-but-unbroadcast transaction was not in it, so two orders shared one nonce; `submit_authorization` now delivers every earlier unexpired SUBMITTING transaction first and answers `503 RELAYER_TRANSACTION_PENDING` (nothing prepared, still signable) when one still will not go. (F03) An RPC failure while checking an expired uncertain broadcast was folded into EXPIRED - a state the sweep never revisits - discarding a receipt the first query had returned; the error is now recorded and raised, the order stays SUBMITTING, and a receipt alone lets the sweep confirm it. (F04) Only the job's own candidate was synced after a batch completion; the worker now syncs every VALIDATING sibling of the job, each failure isolated. (F05) A crash or QA outage after the completion commit stranded candidates VALIDATING with media and no verdict; `resume_interrupted_candidate_qa` runs at worker start (deleted creations excluded). (F06) Any `OSError` on the verifier's read was a permanent INVALID; a read that did not happen now defers (`DEFERRED`, error recorded, the lease re-queues it), only a missing object (`FileNotFoundError`, S3 `NoSuchKey`/404) stays INVALID, and a non-`OSError` store error no longer aborts the batch. (F07) A crash between the completion commit and `_settle_live_generation_fence` left the spend authorization UNCERTAIN forever; `settle_completed_spend_authorizations` runs in `recover_after_restart` and settles from the recorded provider cost (quote when none). (F08) A crash after `prepare_boundary` and before any transport made the retry a forbidden resubmission (`FAILED / LIVE_CANARY_DENIED`); a job still `NOT_SENT` with no provider job now hands the UNCERTAIN hold back and takes it again (budget and permit alike; Flow excluded because provisioning leaves no job marker). (F09) One uncertain direct-API submit flipped the shared `direct-api:<provider>` worker to NEEDS_USER_ACTION, which nothing restores; only browser workers flip now, the job keeps its slot. (F10) A completed upload re-authorized under the same key answered 409; it now replays as its asset, a different scope under the same key is a conflict, and the web app keys each attempt. (F12) `apps/web/nginx.conf` gains `proxy_read_timeout 300s` (was nginx's 60 s default against the api's 120 s model timeout). **Residuals:** an issued PUT stays valid for its TTL after completion (self-overwrite of the uploader's own slot only); OSS-mode direct uploads no longer share one object across projects; a load-balanced RPC whose pending count lags can still hand out a stale nonce; a SUBMITTED transaction that leaves the mempool still has no expiry; a permit-fenced (non-verified) generation that crashes before settlement still waits for the operator; a QA that fails at every start stays VALIDATING with the error logged; the host nginx's own `proxy_read_timeout` is outside the repository (§1.19 carries the password-reset finding, F13). Pinned by new tests in `test_direct_upload.py`, `test_eip3009_relayer.py`, `test_media_verification.py`, `test_batch_candidate_atomicity.py`, `test_production_budget.py`, `test_provider_gateway.py`, `test_reference_upload_contract.py`. | `services/media-service/media_service/direct_upload.py`, `services/media-service/media_service/verification.py`, `core/payments/payment_core/eip3009.py`, `services/generation-gateway/generation_gateway/worker.py`, `services/generation-gateway/generation_gateway/gateway.py`, `apps/api/video_platform_api/main.py`, `apps/web/app.js`, `apps/web/nginx.conf` |
| 2.50 | **Fixed 2026-09-06 — the second production review's seven confirmed findings, and the relayer nonce residual of §2.49.** The eleven-item report was checked against `a1606ed`: its RPC-failure-expires-an-order (§2.49 F03), proxy-timeout (F12) and password-reset (§1.19) items quote the tree before #60/#61 and are already closed there; the rest reproduced. (1) **Relayer nonce.** #60 delivered every earlier unbroadcast transaction before reading the node's pending count, but the count was still the only allocator, so a node whose count lags what it just accepted - a load-balanced RPC, the report's constant-count reproduction - handed one nonce to two orders. `_allocate_relayer_nonce` now takes `max(pending count, 1 + the highest nonce this relayer still holds in the network)` from the rows: a mined row (`CONFIRMED`/`FAILED`/`RECONCILIATION_REQUIRED`) always counts, an unexpired delivered row counts while the node still holds its bytes (`_delivered`), and an expired or dropped one never does - so a nonce gap, which would stall the relayer for good, cannot be built; only rows at or above the count are asked about. `submit_authorization` also holds the process lock across the broadcast, not just the preparation. `last_submitted_nonce` stays informational. (2) **A lost `/submit` answer** made `wallet.js` print "no USDC was transferred" and never ask again, so the user paid twice. Once a signature has been sent, a failure asks `GET …/relayed-authorizations/{id}` first: `SUBMITTING`/`SUBMITTED`/`CONFIRMED` is followed to settlement, `PENDING` keeps the signed order and the next Confirm resends the same signature (no new order, no new signature), an unreachable server leaves the result "pending confirmation" with the order id on the page and keeps polling; "nothing was transferred" is reachable only before signing. (3) **The per-shot spending cap** ("Most I'll spend on this shot (USD)") travelled as `estimated_cost` and was dropped: the pipeline passed `0.0` into the runtime, priced from the catalogue and reserved for the quote. `spend_cap_usd` is now compared with the admission quote before any reservation (`enforce_shot_spend_cap`, 422 with both numbers, nothing charged), rides in `request.metadata` so an automatic retry onto a dearer alternative is refused the same way (`_retry_under_cap`: the plan turns terminal with `SPEND_CAP_EXCEEDED:<quote>><cap>`, the evaluation completes), and the web sends it under its own name; `estimated_cost` remains the caller's estimate. (4) **Sign-out left the previous account's director sessions, session rail, episode strip, continuation draft and prompt boxes** in state and in the DOM; `clearWorkspaceState` now clears them, stops the creative poll, repaints the rails, and every loader that writes private state carries a `workspaceEpoch` fence that drops an answer arriving after the sign-out; the identity strip and the credits pill are cleared with it, and the wallet forgets the payer address, balance and receipt. (5) **A failed `/logout` was treated as a sign-out**: the page hid while the server still held the session and the HttpOnly cookie. Only a confirmed sign-out (or a `401`, which means no server session) locks the page; anything else keeps the account on screen, says so next to the button, and the same button retries. (6) **`POST /v1/assets` was `async def` with no await**, so validation, the full hash and the synchronous object-storage push ran on the event loop and stalled every other request for the length of an upload; it is a plain `def` (threadpool) now, as is `GET /v1/providers`. (7) **The wallet topped up the first owned/admin workspace and the credits pill read the first listed one**, neither the open project's; both bind to `project.workspace_id` now (`ai-director:workspace-changed` on every project switch), the sheet says who receives the credits (`name` on the user view's workspaces, `workspace_name` on billing), and a balance that arrives after a switch is not painted. (8) **Under 1280px the Inspector was `display:none` behind a class nothing set**; `#inspectorToggleBtn` in the top bar sets it, exists only at those widths, and is hidden on pages without an inspector. **Residuals:** no unique index on `(relayer_address, chain_id, relayer_nonce)` - an expired never-delivered row legitimately frees its nonce, so the constraint would refuse correct reuse; a `SUBMITTED` transaction that leaves the mempool is still polled forever (unchanged) but no longer blocks allocation; the Create (passenger) path has no spending cap field; the sign-out retry cannot help a `403` from a missing CSRF cookie, which also breaks every other POST; other `async def` routes that mix sync work with a real `await` (cancel, delete, admin dashboards) were left as they are. Tests: `test_eip3009_relayer.py` (five more), `test_shot_spend_cap.py`, `test_web_session_boundaries.py`, `test_wallet_workspace_binding.py`, `test_reference_upload_contract.py` (route off the loop). | `core/payments/payment_core/eip3009.py`, `core/entitlements/entitlement_core/admission.py`, `core/production/director_production/pipeline.py`, `services/production-engine/production_engine/runtime.py`, `apps/api/video_platform_api/{main,auth,payment_routes}.py`, `apps/web/{app.js,wallet.js,index.html,styles.css}` |
| 2.51 | **Fixed 2026-09-07 — the third production review's twenty-two P2 findings.** Verified against `0d4e71e`; every item reproduced, with one nuance: the "a FREE workspace pins a PRO project" item is a browser defect only - `default_video_role` is already per-project and answers `VIDEO_SEEDANCE` for every plan today - so its effect was latent. (1) **Director stage.** `renderShotStage` showed the approved take or the first variant with output, so a second variant awaiting review could be confirmed unseen. Every variant with output now has *View*; the stage names its take (`#shotStageTake`: "Variant B · Needs your review" / "Approved take · Variant A"); the staged card is marked; a stage render that finishes after a newer one began is dropped; and `humanReviewCandidate` puts the variant on the stage and asks again unless it is the one showing. (2) **Project switching.** `selectProject` takes a sequence number checked after every await, and every project-scoped loader (`loadLogicalAssets`, `loadCharacters`, `loadEpisodeStrip`, `loadEpisode`, `selectShot`, `loadCandidates`, `loadCreativeSessions`, `openCreativeSession`, `refreshProductions`, `loadProjectGenerations`) drops an answer for a project no longer open. (3) **Deletion.** `forgetJob` clears the Create canvas's job, stops its poll and the save dialog, and `rememberJob` refuses ids in `state.forgottenJobs` - the canvas repaint and a shot's candidate list used to remember a deleted creation back. (4)+(5) **Wallet polling.** One engine, `followOrder`, with a generation number: a failed check retries under 5/8/13/20/30 s backoff and parks after six failures behind `#walletRecheckBtn` ("Check payment status again"); Close, "Start creating" and Escape all end at the dialog's `close` event, which - like sign-out and a newer order - bumps the generation so a check still in flight drops its answer; the close `createCheckout` makes for the DePay window is flagged (`widgetHandoff`) and keeps the poll. (6) **Director inspector.** The Framing and Key light pickers nothing read are gone; Angle/Movement stay, labelled as inputs to *Check continuity* only. (7) **Criticality.** The Create picker is gone and `asset_criticality` is no longer sent: admission runs every Create request at STANDARD and the checks never read it. (8) **Plan.** `projectPlanTier()` reads the open project's workspace; Auto sends no `model_role` (the server resolves it from the project's own plan). (9) **Paging.** `GET /v1/generations` takes `before` (keyset on `created_at, id`; 404 for a cursor outside the project) and answers `next_cursor`; Productions gains *Load older creations* and says "N creations loaded" while more exist; the admin console sends `limit`/`offset` and renders Previous/Next from `pagination`. (10) **Admin deep links** map route names to detail kinds (`DETAIL_KIND`). (11) **Audit rows** open from the record on the page, or `GET /api/admin/audit?id=` for a deep link. (12) **Credits line.** `creditLine` knows reserved / charged / refunded / held-for-a-check (and "quoted" when the ledger is unknown). (13) **Retry.** `generation_gateway.job_allowed_actions` is the one rule - exactly `retry()`'s and `cancel()`'s preconditions - and rides on the job view, the submit answer and the admin detail as `allowed_actions`; both surfaces read it, so a finished failure offers nothing but "start a new one from Create". (14) **Pricing page.** `GET /v1/payments/catalog` (public) serves the checkout's packs; `public.js` keeps the same rows as fallback and a test pins the two together. (15) Key visuals go through `cachedAssetThumbnail`. (16) `body.admin-route` scrolls the document. (17) The director reply skips Enter while `isComposing` (or keyCode 229). (18) `_asset_public_url` mints `{public_base_url}/v1/storage/{key}` for a row with none (a direct upload); `resolveAssetMedia` reads the key whenever there is no address and the Create canvas uses that resolver. (19) `processCommand`'s BUSY heartbeat runs inside the try/finally. (20) `nginx.conf` maps `$http_upgrade` and forwards `Upgrade`/`Connection` on `/api/`. (21) `_job_view` and the submit answer carry `duration` (what runs), `requested_duration`, `aspect_ratio` and `resolution`; the canvas shows "6s (asked 5s)" and the submit toast says so. (22) An approved shot's button reads "Approved · in the timeline", disabled - the timeline fence admits DRAFT/PLANNED/READY only. **Residuals:** the host nginx in front of the container gained the same two headers plus an http-level map on 2026-09-07 ≈11:51Z (outside the repository; `docs/DEPLOYMENT.md` §6 carries the proof, and a WebSocket probe there must use `curl --http1.1` because HTTP/2 has no `Upgrade` header); a relayed order signed while the sheet is closing is still followed to settlement, on purpose; a criticality for the Create path stays a product decision; the shot's own `duration` remains the requested one (execution length is per candidate); the admin pager is offset-based, like its endpoints. Tests: `test_web_review_fixes_2026_09_07.py`, `test_wallet_polling_recovery.py`, `test_review_fixes_2026_09_07_api.py`, `test_wallet_workspace_binding.py` (harnesses lift the engine). | `services/generation-gateway/generation_gateway/{gateway,__init__}.py`, `apps/api/video_platform_api/{main,admin_routes,runtime_routes,payment_routes}.py`, `apps/web/{app.js,wallet.js,admin.js,public.js,index.html,styles.css,admin.css,public.css,nginx.conf}`, `apps/browser-worker-extension/background.js` |
| 2.52 | **Fixed 2026-09-08 — Skill Registry ≠ Runtime Invocation.** Audit: twelve Skills registered and displayed, but only the director body entered a model call (turns and the whole screenplay, shots included), the prompt-compiler was resolved for its hash and never injected, shot decomposition / cinematography / continuity had no Skill call, the continuation used a hard-coded prompt, the corrector record claimed `"v1"`, and a model failure's fallback still carried the Skill's version. Fix: `core/skills/skill_core/runtime.py` (`SkillRuntime`: resolve one operation → one Skill from machine-readable frontmatter, inject that body alone, validate the contract, record `resolved / loaded / model_invoked / execution_mode / fallback_reason`); frontmatter on all twelve `SKILL.md` (role, stage, runtime, operations, model_role, authority, forbidden_authority, output_contract, bound_to) and the ten sections (Purpose … Unresolved Policy); the screenplay split into a Director story (`StoryDraft`, no shot fields - stripped and recorded) and a Shot Planner plan (`ShotPlan`, new `ModelRole.SHOT_PLANNER`, a plan that changes a line or invents a character is rejected whole) merged by `merge_story_and_shot_plan`; `CinematographyDesigner` (`shots.cinematography_json`, migration `0082`, framing written back to `shot_type`, read by the compiler), `ContinuityReviewer` (`CONTINUITY_REVIEW` decisions, `PASS/REPAIRABLE/ESCALATE`), `compile_shot_with_skill` (preflight, re-verification, hash-keyed reuse on the generation path, a Skill `NOT_COMPILABLE` honoured); `VisualStageRunner` after beats approval (`FEATURE_SKILL_STAGES_AT_APPROVAL`); the continuation under the director Skill; invariant versioning on every revision (`invariant_version`, `supersedes_version`, `invariant_changes`); `GET /v1/skills` with bindings, `GET /v1/skills/runtime` with the matrix, per-shot stage routes. **Residuals:** (1) **live check done 2026-09-09 on dev** (`docs/SKILL_RUNTIME.md` §8): seven calls, all answered by the FREE binding `doubao-seed-2-0-lite` (the project's workspace is on the FREE plan, as a free user's would be); director turns and story Skill-driven, the shot plan rejected for micro-actions outside the closed vocabulary (tracked fallback), the cinematography plan rejected for a 200-character `camera.focus` (tracked fallback), continuity `ESCALATE` on absent end-frame evidence, the compiler `NOT_COMPILABLE` on a first shot whose `subjects` is empty - four contract follow-ups listed there - (1)-(3) landed the same day (advisory micro-actions, wider cinematography limits, the first shot's actor and prop from its output state, provenance on the record) together with a 12k / 16k output cap for the story / shot plan and a `MODEL_OUTPUT_TRUNCATED` code - and the PRO-workspace run (§9 there) then answered with opus / sonnet / gpt-5.6-sol: director, story, cinematography, continuity Skill-driven, the shot plan Skill-driven after the cap fix, the compiler refusing a script-only shot whose cinematography plan hedged its framing; (2) beats approval is now ≈2N+1 small paid calls per N-shot episode under live mode; (3) the deterministic continuity review compares timeline states only, never the textual director states; (4) shot size lives on `shot_type` only after cinematography designs the shot - a shot generated before that stage runs uses the compiler's locked-off defaults, on record; (5) `image-prompt-corrector` and `model-prompting` stay reference-only by design. Tests: `tests/test_skill_runtime.py` (28), `tests/test_creative_director.py` and `tests/test_installed_skills.py` updated. | `core/skills/`, `skills/*/SKILL.md`, `core/creative-director/creative_director_core/{schemas,screenplay,director_context,service}.py`, `core/production/director_production/{cinematography,stages}.py`, `core/continuity/continuity_core/review.py`, `core/video-prompt/video_prompt_core/compiler.py`, `core/episode-continuation/episode_continuation_core/service.py`, `packages/contracts/platform_contracts/{cinematography,continuity}.py`, `core/model-registry/model_registry_core/schemas.py`, `config/model-registry/defaults.json`, `migrations/versions/0082_shot_cinematography_plan.py`, `apps/api/video_platform_api/{container,main,skill_routes,creative_routes}.py`, `apps/web/app.js`, `scripts/{skill_runtime_matrix,review_skill_contract}.py` |
| 2.53 | **Fixed 2026-09-09 — the Skill runtime's content-quality acceptance.** The merged #64 + #65 tree routed every Skill through the runtime and still let a paid decision stop short of the output: (1) **the prompt-compiler Skill's package never reached the provider** - `prepare_autopilot` gave the adapter the spec alone and the adapter rebuilt the prompt from `canonical_lines` with a fixed negative prompt, so the Skill's wording lived only in `prompt_compilations` / `shots.compiled_prompt`; now `AdapterInput.package` makes the package the body every adapter delivers (its model-specific lines, the locked style, unrestated continuity assertions and the bounded context after it; the negative prompt merged with the baseline guards), `metadata.prompt_delivery` records the source (`SKILL_PACKAGE` / `ADAPTER_CANONICAL`, replay-stable facts only) and `metadata.prompt_package` lets a retry onto another model keep the wording; (2) **a Skill-driven continuity `ESCALATE` + `approval_required` was recorded and never enforced** - now `ContinuityReviewer.pending_escalation` (the newest Skill-driven review, unless a later Skill-driven review replaced it or a `CONTINUITY_ESCALATION_ACKNOWLEDGED` decision names it; deterministic reviews are advisory) gates `compile()` (`ContinuityApprovalRequired`, 409 `CONTINUITY_APPROVAL_REQUIRED`), `compile_shot_with_skill()` (recorded `NOT_COMPILABLE`, no reuse, no call), the stage report (`approval_required`) and `POST /v1/shots/{id}/generate` (before any planning or reservation; a stale escalation is reviewed again once, a fallback re-review keeps it pending); a real user releases it with `POST /v1/shots/{id}/continuity/review/acknowledge` (bypass → 403), `GET …/continuity/review` shows the gate; every review decision carries its `input_hash`; (3) **unresolved photography compiled** - the preflight now covers every camera / lighting / subject / prop field and the aspect ratio, refuses markers and word-bounded hedges (`provisional`, `tentative`, `暂定`, …) in photographic fields only, and carries a Skill-driven cinematography plan's `unresolved` entries into the envelope as `cinematography.unresolved[n]` (a deterministic plan's marker stays a record), so the deterministic fallback can no longer complete what the model refused; (4) **the plan's height, lens intent, depth of field, motivation, exposure intent, subject placement, atmosphere and compositions were dropped** - new fields on `CanonicalCameraSpec` / `CanonicalLightingSpec` / `CanonicalShotSpec`, read by the envelope (`subject_positions` through `identity_key`, filling a subject whose approved state has no explicit `screen_position`, applied / kept / unmatched on the record), rendered by the neutral prompt, the checklist and every adapter only when set; (5) **the fresh-package cache ignored the Skill version** - freshness now keys on the producing Skill's content hash (`SKILL_VERSION_CHANGED:<old>` on a stale one; the stage re-asks the model; records name the producing Skill; a Skill `NOT_COMPILABLE` survives a Skill edit until the new Skill answers), and a cinematography fallback keeps an older Skill's plan while the decision says so. **Consequences:** every stored Skill package is stale once at deploy (new spec keys + three edited Skill bodies: one paid prompt-compiler call per shot at its next generate, deterministic before that; a same-key generate straddling the deploy is a 409); a same-key replay after an escalation was recorded, or across a deterministic→Skill compile, conflicts; a plan-level unresolved entry is released only by changing the shot and re-running the stage; no web affordance yet for the acknowledgement (the 409 message names the route). An adversarial review of the fixes themselves (five reviewers, three skeptics per finding) added ten
corrections before the branch was committed, the largest being that the Skill path dropped the shot's
`constraints` entirely - a client prohibition or the director's approved staging could be lost exactly when
the paid compiler wrote the prompt, since only the action, subjects, line, claims and copy are re-verified;
`prompt_lines` now appends the unrestated constraints and the negative prompt merges the shot's forbidden
terms. Also: `prompt_delivery` no longer carries the installed Skill's identity on the deterministic path
(the gateway hashes it, so a `SKILL.md` edit would have 409'd every in-flight idempotency key); the preflight
scans `atmosphere` and `composition`; a deterministic cinematography plan no longer fills the new camera
design fields with the contract's defaults; a fallback re-review runs once per change of inputs instead of
once per generate attempt; the cinematography stage's own write-back (`shot_type`) left its reuse key, so a
...
| # | Item | Blocked by |
| --- | --- | --- |
| 3.1 | **Done 2026-09-08 — model-backed prompt compilation.** `compile_shot_with_skill` runs the prompt-compiler Skill through the runtime with the deterministic preflight and re-verification; the generation path reuses a fresh package and compiles deterministically otherwise, on record. Residual: no live PROMPT_COMPILER call has been observed yet (§2.52). | — |
...
| 3.8 | **A creation-media cleanup row that exhausts its attempts waits silently.** Deleting a creation queues `creation_media_cleanups`; a bucket that stays unreachable retries under a doubling backoff and then parks at `FAILED` with its last error. Nothing alerts on that state today — an operator has to look, then re-drive it with `POST /internal/maintenance/creation-media?cleanup_id=…&include_failed=true`. The deletion itself is unaffected (it committed long before), so this is retained bytes, not a visible defect. | `services/media-service/media_service/creation_cleanup.py` |
